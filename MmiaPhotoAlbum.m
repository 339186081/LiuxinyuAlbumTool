...
- (void)showPhotos
{
    if (self.imageArray.count == 0) {
        ...
        if (nextIndex >= _imageArray.count) {
            nextIndex = _imageArray.count - 1;
        }
...
- (void)loadImageNearIndex:(NSInteger)index
{
    if (index > 0) {
        ...
        [[SDWebImageManager sharedManager]downloadImageWithURL:url options:SDWebImageLowPriority | SDWebImageRetryFailed progress:^(NSInteger receivedSize, NSInteger expectedSize) {} completed:^(UIImage * image, NSError * error, SDImageCacheType cacheType, BOOL finished, NSURL * imageURL) {}];
    }
